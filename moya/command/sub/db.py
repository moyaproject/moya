...
class DB(SubCommand):
    ...
    def add_arguments(self, parser):
        ...
        subparsers = parser.add_subparsers(
            title="db sub-commands", dest="dbsubcommand", help="database action"
        )
        ...
        parser = subparsers.add_parser(
            "sync", help="sync database(s)", description="create tables from models"
        )
        add_common(parser)
    ...
    def sub_list(self):
        ...
        table += sorted(
            [(name, engine.engine_name) for name, engine in engines.items()],
            key=lambda d: d[0],
        )
    ...
    def sub_validate(self):
        ...
        build_result = build_server(self.location, self.get_settings())
