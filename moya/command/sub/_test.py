...
class TestResults(object):
    ...
    def add_pass(self, element):
        self._test_result("pass", element=element, description=self.case.description)

    def add_fail(self, element, msg):
        self._test_result(
            "fail", element=element, msg=msg, description=self.case.description
        )
    ...
    def report(self, console, show_trace=True, show_passes=False, verbose=False):
        def show_result(result, description, msg=None):
            if result == "pass":
                ...
                fg = "red"
            console("[{}]\t".format(result.upper()), fg=fg, bold=result != "pass")(
                " {}".format(description), dim=result == "pass"
            )
            if msg:
        ...
        if verbose:
            console.div(
                "[{}]".format(self.suite), fg="magenta", bold=True, italic=False
            )
        ...
        for test in self.tests:
            ...
            if status == "fail" and show_trace:
                ...
                console.xmlsnippet(
                    node._code, node.source_line or 0, extralines=2 if verbose else 0
                )
            if status != "pass":
                show_output(test)
...
class Test(SubCommand):
    ...
    def run(self):
            ...
            for suite_no, suite in enumerate(suites, 1):
                ...
                if args.quick and suite.slow:
                    with self.console.progress(
                        "{} {} (skipped slow test)".format(lib, suite_description), 0
                    ):
                        pass
                    continue
                ...
                if (
                    args.exclude
                    and suite.group is not None
                    and suite.group in args.exclude
                ):
                    with self.console.progress(
                        "{} {} (excluded group)".format(lib, suite_description), 0
                    ):
                        pass
                    continue
                ...
                if args.group and suite.group not in args.group:
                    with self.console.progress(
                        "{} {} (not in group)".format(lib, suite_description), 0
                    ):
                        pass
                    continue
                    ...
                    if steps == 0:
                        ...
                        try:
                            ...
                            with TestRunner(context, results):
                                for test in tests:
                                    results.case = test
                                    test_callable = archive.get_callable_from_element(
                                        test
                                    )
                                    try:
        ...
        summary = "{fail} fail(s), {error} error(s), {pass} pass(es)".format(
            **all_totals
        )
        ...
        console.text(
            "Ran {} test(s) in {} test suite(s) - {}".format(
                test_count, len(all_results), summary
            )
        )
        ...
        summary = "{fails} fail(s), {errors} error(s), {passes} pass(es)".format(
            fails=fails, errors=errors, passes=passes
        )
        if errors or fails:
            console.text(summary, fg="red", bold=True)
        else:
