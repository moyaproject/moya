...
class Runserver(SubCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "-p", "--port", dest="port", default="8000", help="port to listen on"
        )
        parser.add_argument(
            "-b",
            "--breakpoint",
    ...
    def run(self):
        ...
        try:
            ...
            if e.errno in (48, 98):
                log.critical(
                    "couldn't run moya server, another process may be running on port %s",
                    args.port,
                )
            raise
        ...
        log.info(
            "development server started on http://{}:{}".format(args.host, args.port)
        )
