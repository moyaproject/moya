...
class Trace(SubCommand):
    ...
    def run(self):
        ...
        count = 0
        for route_data, (app, element) in application.server.trace(
            application.archive, args.url, args.method
        ):
            count += 1
            self.console.div()
            self.console("In file ")('"%s"' % element._location).nl()
            self.console.table(sorted(route_data.items()), header=False, dividers=False)
            self.console.xmlsnippet(
                element._code, element.source_line or 0, extralines=3
            )
            if args.step:
                raw_input()

        self.console.nl()(
            "{} {}".format(args.method, args.url),
            bold=True,
            fg="magenta" if args.method == "GET" else "blue",
        )(" maps to ")(text_type(count), bold=True)(" target(s)").nl()
