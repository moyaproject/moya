...
"""
    ...
    def _get_moya_libs(self):
        ...
        moya_libs = {
            name: version
            for name, version, _, _ in archive.Archive.scan_libs(
                [archive.MOYA_LIBS_PATH], None
            )
        }
        return moya_libs
    ...
    def get_argparse(self):
        ...
        build_parser = subparsers.add_parser(
            "build", help="build a package", description="Build a library"
        )

        build_parser.add_argument(
            dest="location", metavar="PATH", help="Library location"
        )
        build_parser.add_argument(
            "-f",
            "--force",
        ...
        upload_parser = subparsers.add_parser(
            "upload", help="upload a package", description="Upload a package"
        )

        upload_parser.add_argument(dest="location", help="Library location")
        ...
        install_parser.add_argument(
            "--app", dest="app", default=None, help="name of app to install"
        )
        install_parser.add_argument(
            "--no-add",
            dest="no_add",
    ...
    def run_auth(self):
        ...
        if username is None:
            ...
            if not username:
                self.console.error(
                    "a username is required (see http://packages.moyaproject.com)"
                )
                return
    ...
    def run_register(self):
        ...
        def get(section, key, default=Ellipsis):
            try:
                ...
                raise CommandError(
                    "key [{}]/{} was not found in lib.ini".format(section, key)
                )
    ...
    def run_build(self):
        ...
        if not args.force and package_destination_fs.exists(package_filename):
            raise CommandError(
                "package '{}' exists, use --force to overwrite".format(package_filename)
            )
    ...
    def upload(self, url, package_name, version, build_fs, filename, overwrite=False):
        ...
        if not overwrite:
            try:
                self.call(
                    "package.get-download-info", package=package_name, version=version
                )
                ...
                raise CommandError(
                    """Upload failed because this release exists. It generally better to create a new release than overwrite an existing one.\n"""
        ...
        with build_fs.open(filename, "rb") as package_file:
            ...
            response = requests.post(
                url, verify=False, auth=auth, files=files, data=data, hooks={}
            )
    ...
    def run_upload(self):
        ...
        if not package_destination_fs.exists(package_filename):
            raise CommandError(
                "package '{}' does not exist, run 'moya-pm build'".format(
                    package_filename
                )
            )
    ...
    def upload_docs(self, lib_name, lib_version):
        ...
        with io.open(temp_filename, "rb") as package_file:
            files = [
                ("file", (package_filename, package_file, "application/octet-stream"))
            ]
            data = {
                "auth": self.auth_token,
                "package": lib_name,
            ...
            response = requests.post(
                docs_url, verify=False, files=files, data=data, hooks={}
            )
    ...
    def run_list(self):
        ...
        releases = self.call("package.list-releases", package=version_spec.name)[
            "releases"
        ]
        ...
        for release in releases:
            if version_spec.comparisons and release["version"] not in version_spec:
                continue
            table.append(
                [Cell(release["version"], bold=True, fg="magenta"), release["notes"]]
            )
    ...
    def select_packages(self, packages):
        ...
        for _package, package_select in selected:
            if package_select is None:
                version_spec = versioning.VersionSpec(_package)
                raise CommandError(
                    "requested package '{}' was not found".format(version_spec.name)
                )
            if package_select["version"] is None:
                raise CommandError(
                    "no installation candidate for '{}'".format(_package)
                )
        return selected
    ...
    def check_existing(self, package_installs):
        ...
        if not (self.args.force or self.args.download):
            try:
                ...
                if archive is None:
                    raise CommandError(
                        "unable to load project, use the --force switch to force installation"
                    )
            except Exception as e:
            ...
            for package_name, install_version in package_installs:
                ...
                libs = [
                    (lib.long_name, lib.version, lib.install_location)
                    for lib in archive.libs.values()
                    if lib.long_name == package_name
                ]
                ...
                for name, version, location in libs:
                    if name == package_name:
                        if version > install_version:
                            raise CommandError(
                                "a newer version ({}) of package {} is already installed, use --force to force installation".format(
                                    version, name
                                )
                            )
                        elif install_version == version:
                            raise CommandError(
                                "version {} of {} is already installed, use --force to force installation".format(
                                    version, name
                                )
                            )
                        else:
                            if not self.args.upgrade:
                                raise CommandError(
                                    "an older version ({}) of {} is installed, use --upgrade to force upgrade".format(
                                        version, name
                                    )
                                )
            return application
    ...
    def install_packages(self, output_fs, selected_packages, application=None):
        ...
        for index, (_, select_package) in enumerate(selected_packages):
            app_name = self.args.app or select_package["name"].split(".", 1)[
                -1
            ].replace(".", "")

            _install = self.download_package(
                download_fs,
                select_package,
        ...
        for _package in install_packages:
            _changed_server, _installed_packages = self.install_package(
                download_fs, output_fs, _package, cfg=cfg
            )
        ...
        for _package, mount in installed:
            table.append(
                [
                    Cell("{name}".format(**_package), fg="magenta", bold=True),
        ...
        if application is not None:
            ...
            if changed_server:
                self.console.text(
                    "moya-pm modified '{}' -- please check changes".format(server_xml),
                    fg="green",
                    bold="yes",
                )
    ...
    def install_package(self, download_fs, output_fs, packages, cfg=None):
        ...
        for package_name, (app_name, mount, package_select) in packages.items():
            ...
            if package_select["system"]:
                ...
                filename = "{}-{}.{}".format(
                    package_name, install_version, package_select["md5"]
                )
            ...
            with download_fs.open(filename, "rb") as package_file:
                with ZipFS(package_file) as package_fs:
                    with output_fs.makedir(
                        package_select["name"], recreate=True
                    ) as lib_fs:
                        lib_fs.removetree("/")
    ...
    def download_package(self, download_fs, select_package, app=None, mount=None):
        ...
        packages = dependencies.gather_dependencies(
            self.rpc,
            app,
            mount,
            _install,
        ...
        if not args.no_add:
            for package_name, (app_name, mount, package_select) in packages.items():
                if package_select["version"] is None:
                    raise CommandError(
                        "no install candidate for dependency '{}', run 'moya-pm list {}' to see available packages".format(
        ...
        for package_name, (app_name, mount, package_select) in packages.items():
            ...
            with download_fs.open(filename, "wb") as package_file:
                ...
                if checksum != package_select["md5"]:
                    raise CommandError(
                        "md5 checksum of download doesn't match server! download={}, server={}".format(
                            checksum, package_select["md5"]
                        )
                    )
    ...
    def run_install(self):
        ...
        package_installs = [
            (p["name"], versioning.Version(p["version"]))
        ...
        output_path = (
            ...
            else join(self.location, args.output)
        )
        ...
        for name, package in selected_packages:
            if package["notes"]:
                self.console.table(
                    [[package["notes"]]],
                    ["{name} {version} release notes".format(**package)],
                )
