...
class MoyaDoc(object):
    ...
    def get_argparse(self):
        parser = argparse.ArgumentParser(prog="moya-doc", description=self.__doc__)
        parser.add_argument(
            "--version",
            dest="version",
            metavar="MAJOR.MINOR",
        ...
        parser.add_argument(
            "--build", "-b", dest="build", action="store_true", help="Build HTML docs"
        )
        parser.add_argument(
            "--no-browser",
            "-n",
        ...
        def do_extract():
            ...
            with extract_fs.opendir("site/tags") as tags_fs:
                ...
                for namespace in self.builtin_namespaces:
                    ...
                    if xmlns is None:
                        raise ValueError(
                            "XML namespace '{}' is not in namespaces.py".format(
                                namespace
                            )
                        )
                ...
                for language in languages:
                    ...
                    with extract_fs.opendir("site") as extract_site_fs:
                        ...
                        with timer("render time"):
                            builder.build({"libs": lib_info, "lib_paths": lib_paths})
