...
class AttributeExposer(object):
    ...
    def items(self):
        return [
            (k.lstrip("_"), getattr(self, k)) for k in self.__moya_exposed_attributes__
        ]
    ...
    def __contains__(self, k):
        return (
            k in self.__moya_exposed_attributes__
            or ("_" + k) in self.__moya_exposed_attributes__
        )
...
class ObjectExposer(AttributeExposer):
    ...
    def __moya_exposed_attributes__(self):
        if not hasattr(self, "_exposed_attributes"):
            self._exposed_attributes = sorted(
                [
                    k
                    for k in dir(self)
