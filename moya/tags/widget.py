    ...
    def logic(self, context):
        ...
        if "_return" in scope:
            ...
            if not hasattr(scope, "items"):
                self.throw(
                    "widget.return-no-dict",
                    "the return value from a widget must be a dict, or None (not {})".format(
                        context.to_expr(scope)
                    ),
                )
...
class Widget(ElementBase):
    ...
    container = Attribute(
        "Is this widget a container?", type="boolean", default=True, required=False
    )
    synopsis = Attribute("Short description of the widget")
    undocumented = Attribute(
        "Set to yes to disabled documentation for this tag",
        type="boolean",
        default=False,
    )
    ...
    cachekey = Attribute(
        "Cache key", name="cachekey", type="text", default=None, required=False
    )
    cachefor = Attribute(
        "Cache time", name="cachefor", type="timespan", default=None, required=False
    )
    ...
    def finalize(self, context):
        ...
        for signature in self.children("signature"):
            for attribute_tag in signature.children("attribute"):
                ...
                if attribute_tag.has_parameter(
                    "default"
                ) and not attribute_tag.has_parameter("required"):
                    param_map["required"] = False
                description = attribute_tag.doc
                name = param_map.pop("name")
                attribute = Attribute(
                    description, name=name, evaldefault=True, **param_map
                )
                attributes[attribute.name] = attribute

        # if 'from' not in attributes:
        attributes["from"] = Attribute(
            "Application", name="from", type="application", map_to="from", default=None
        )
...
class AttributeTag(ElementBase):
    ...
    type = Attribute(
        ...
        required=False,
        default="expression",
        choices=attributetypes.valid_types,
    )
    ...
    missing = Attribute(
        "Are missing values allowed?", type="boolean", default=True, required=False
    )
    empty = Attribute(
        "Are empty values allowed?", type="boolean", default=True, required=False
    )
    choices = Attribute(
...
class ArgumentTag(ElementBase):
    ...
    check = Attribute(
        "A boolean expression that the attribute must satisfy",
        type="function",
        default=None,
    )
    default = Attribute(
        "A value to use if the argument is not supplied", type="function", default=None
    )
        ...
        for arg in element.children():
            if arg._element_type != (namespaces.default, "argument"):
                raise errors.ElementError(
                    "{} signature must contain <argument> tags only".format(
                        element.parent
                    ),
                    element=element,
                )
            name, required, check, default = arg.get_parameters(
                context, "name", "required", "check", "default"
            )
    ...
    def check(self, context, arg_map, checked_object):
        ...
        if not self.arg_names.issuperset(arg_map.keys()):
            for k in self.arg_names:
                if k not in arg_map:
                    raise ValueError(
                        "'{}' is a required argument to {}".format(k, checked_object)
                    )
        ...
        for name, check in self.checks:
            try:
                ...
                raise ValueError(
                    "check failed for argument '{}' with error '{}'".format(name, e)
                )
            if not result:
                raise ValueError(
                    "{value} is not a valid value for argument {name}".format(
                        name=name, value=to_expression(context, arg_map[name])
                    )
                )
    ...
    def validate(self, context, element, arg_map):
            ...
            for k in arg_map:
                if k not in self.arg_names:
                    ...
                    if nearest is not None:
                        ...
                        diagnosis = "Valid arguments to this macro are {}.".format(
                            textual_list(sorted(self.arg_names), "and")
                        )
                    element.throw(
                        "argument-error.unknown-argument",
                        "no argument called '{name}' in {element}'".format(
                            name=k, element=element
                        ),
                        diagnosis=diagnosis,
                    )
        if not self.required_set.issubset(arg_map.keys()):
            for name in self.required:
                if name not in arg_map:
                    element.throw(
                        "argument-error.required",
                        "argument '{}' is required in {}".format(name, element),
                        diagnosis="""You can pass a value for '{name}' with let:{name}="<VALUE>" """.format(
        ...
        for name, check in self.checks:
            try:
                ...
                element.throw(
                    "argument-error.check-error",
                    "check for argument '{}' in {} failed with exception: {}".format(
                        name, element, e
                    ),
                    diagnosis="An exception was thrown when evaluating the expression '{}'.\n\n"
                    "This could indicate a programming error in the macro or Moya.".format(
                        check.expression
                    ),
                )
            if not result:
                ...
                element.throw(
                    "argument-error.check-failed",
                    """argument '{}' failed check "{}" in {}""".format(
                        name, check.expression, element
                    ),
...
class Signature(ElementBase):
    ...
    def finalize(self, context):
        if self.parent._element_type in (
            (namespaces.default, "macro"),
            (namespaces.default, "Filter"),
        ):
