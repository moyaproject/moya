...
class Read(DataSetterBase):
    ...
    filename = Attribute(
        "Filename to associate with the image if the filename can't be detected",
        default="",
        type="expression",
    )
    ...
    def get_image(self, context, params):
        if self.has_parameter("file"):
            ...
            try:
                ...
                self.throw(
                    "image.read-fail", "Failed to read image from file ({})".format(e)
                )
        else:
            if params.fsobj is not None:
                ...
                try:
                    ...
                    self.throw(
                        "image.no-fs", "No filesystem called '{}'".format(params.fs)
                    )
            if params.path is None:
                self.throw(
                    "image.path-required",
                    "a path is required when reading from a filesystem",
                )
            try:
                ...
                self.throw(
                    "image.read-fail",
                    "failed to read '{}' from {!r} ({})".format(params.path, fs, e),
                )
        return img
    ...
    def logic(self, context):
        ...
        moya_image = MoyaImage(
            img, filename=basename(params.path or params.filename or "")
        )
...
class CheckImageMixin(object):
    ...
    def check_image(self, context, image):
        if not isinstance(image, MoyaImage):
            ...
            self.throw("bad-value.image", _msg.format(context.to_expr(image)))
...
class Write(ImageElement):
    ...
    def logic(self, context):
        ...
        with params.image._lock:
            ...
            img_format = (
                params.format
                or splitext(params.filename or "")[-1].lstrip(".")
                or "jpeg"
            )
            ...
            try:
                ...
                self.throw(
                    "image.write-fail",
                    "Failed to write {} to '{}' in {!r} ({})".format(
                        params.image, path, fs, e
                    ),
                )
...
class Copy(DataSetterBase, CheckImageMixin):
    ...
    image = Attribute(
        "Image to copy", type="expression", default="image", evaldefault=True
    )
    dst = Attribute("Destination", type="reference", default="image")
...
class Show(ImageElement):
    ...
    image = Attribute(
        "Image to show", type="expression", default="image", evaldefault=True
    )
...
class ResizeToFit(ImageElement):
    ...
    resample = Attribute(
        "Method for resampling", default="antialias", choices=_resample_methods.keys()
    )
...
class ResizeCanvas(ImageElement):
    ...
    def logic(self, context):
        image, w, h, color = self.get_parameters(
            context, "image", "width", "height", "color"
        )
...
class Square(ImageElement):
    ...
    image = Attribute(
        "Image to crop", type="expression", default="image", evaldefault=True
    )
...
class Crop(ImageElement):
    ...
    box = Attribute(
        "Crop size (either [width, height] or [x, y, width, height])", type="expression"
    )
    ...
    def logic(self, context):
        ...
        with params.image._lock:
            ...
            if len(size) == 2:
                ...
                self.throw(
                    "bad-value.box-invalid",
                    "parameter 'box' should be  sequence of 2 or 4 parameters (not {})".format(
                        context.to_expr(size)
                    ),
                )
