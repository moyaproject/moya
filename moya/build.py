...
ServerBuildResult = namedtuple("ServerBuildResult", ["archive", "context", "server"])
...
def build_lib(
    location, archive=None, dependancies=None, ignore_errors=False, tests=False
):
...
def build_server(
    ...
    try:
        with Expression._lock:
            (project_fs, archive, context, doc) = build(
                fs,
                settings_path,
                rebuild=rebuild,
                strict=strict,
    ...
    try:
        ...
        raise errors.StartupFailedError(
            "no <server> element called '{}' found in the project (check setting [project]/startup)".format(
                server_element
            )
        )
    error_msg = None
    ...
    try:
        with pilot.manage_request(None, context):
            server.startup(
                archive,
                context,
                project_fs.opendir(docs_location),
    ...
    if validate_db:
        ...
        if db.validate_all(archive, console) == 0:
            ...
            msg = (
                "Models failed to validate, see 'moya db validate' for more information"
            )
