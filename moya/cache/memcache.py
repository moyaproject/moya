...
class MemCache(Cache):
    ...
    def __init__(
        ...
        super(MemCache, self).__init__(
            name, namespace, compress=False, thread_safe=True
        )
        if pylibmc is None:
            raise errors.StartupFailedError(
                "'pylibmc' module is required for cache type 'memcache' (see https://pypi.python.org/pypi/pylibmc)"
            )

        if not hosts:
            log.warning(
                "{} no 'hosts' setting supplied for memcache server, using '127.0.0.1'".format(
                    self
                )
            )
            hosts = ["127.0.0.1"]
        mc = pylibmc.Client(
            hosts, binary=True, behaviors={"tcp_nodelay": True, "ketama": True}
        )
