...
class H1Tag(BlockTag):
    ...
    def render_close(self, data, text=None):
        ...
        return h_format.format(
            text=html_escape(text), level=self.level + 1, anchor=slugify(title)
        )
...
class DocLinkTag(Tag):
    ...
    def render_open(self, data):
        ...
        return """<a href="{url}" title="{title}">""".format(
            url=relativefrom(path, url), title=title
        )
...
class BBCode(object):
            ...
            if tag_type == TOKEN_TEXT:
                ...
                if end_tag:
                    try:
                        ...
                        raise ValueError(
                            "end tag {} doesn't match an opening tag".format(tag_token)
                        )
                else:
    ...
    def render(self, text, data=None, path="?"):
        ...
        def raise_error(text):
            raise BBCodeError(
                'File "{}", line {}, col {}: {}'.format(path, line + 1, col + 1, text)
            )
