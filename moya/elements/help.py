...
def help(archive, console, tagname):
    ...
    details = [
        (Cell("name", bold=True), tagname),
        (
            Cell("synopsis", bold=True),
            getattr(tag.Help, "synopsis", None) if hasattr(tag, "Help") else "",
        ),
    ...
    for name, attrib in tag._tag_attributes.items():
        if name not in base_attributes:
            params.append(
                (
                    Cell('"{}"'.format(name), fg="cyan"),
                    attrib.doc,
                    attrib.type_display.lower(),
                    "Yes" if attrib.required else "No",
                    attrib.default_display(attrib.default)
    ...
    if params:
        ...
        console.table(
            make_table_header("attrib", "doc", "type", "required?", "default") + params
        ).nl()
